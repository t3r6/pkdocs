...
7. Optional. You can change the way you gain speed from rockets when using rocket jumps with the **Forward Rocket Jump** standard Painkiller feature. Forward Rocket Jump is usually turned off on PK++ servers. However, it might be fun using this feature on a trick jump server. Run the following command to trun it on:
